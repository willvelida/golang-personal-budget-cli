...
func (b *Budget) RemoveItem(description string) {
	for i := range b.Items {
		if b.Items[i].Description == description {
			// We'll append each individual element from a sub-slice, starting from the element immediately after the one we want to remove, to a sub-slice composed of all elements that come before that one.
